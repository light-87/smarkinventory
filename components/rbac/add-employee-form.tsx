      ...
      <CardBody className="flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          {(["employee", "accountant"] as const).map((r) => (
            <label
              key={r}
              className="flex min-h-11 flex-1 cursor-pointer items-center justify-center gap-2 rounded-lg border border-charcoal px-3 py-2 text-[13px] capitalize text-snow has-[:checked]:border-smark-orange has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-50"
            >
              <input
                type="radio"
                name="role"
