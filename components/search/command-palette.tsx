          ...
          >
            ...
            <div className="max-h-[60vh] overflow-y-auto p-2">
              ...
              {pending && !showHint && flatItems.length === 0 && (
                <p className="px-3 py-6 text-center text-[13px] text-smoke">Searching…</p>
              )}
              {showEmpty && <p className="px-3 py-6 text-center text-[13px] text-smoke">No matches for “{trimmedQuery}”</p>}

              {flatItems.map((item, index) => (
                <Fragment key={item.key}>
                  {item.sectionLabel && (
                    <div className="px-3 pt-3 pb-1 text-[11px] tracking-[0.06em] text-smoke uppercase">{item.sectionLabel}</div>
                  )}
                  <button
                    type="button"
            ...
            <div className="border-t border-charcoal px-4 py-2 text-[11px] text-faint">↑↓ Navigate · ↵ Open · Esc Close</div>
          </div>
        </>
      )}
    </>
  ...
  return (
    <>
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate font-mono text-[13px] text-snow">{label}</span>
        {meta && <span className="block truncate text-[12px] text-smoke">{meta}</span>}
      </span>
...
function PartRow({ part }: { part: PalettePartHit }) {
  ...
  return (
    <>
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate font-mono text-[13px] text-snow">{part.internal_pid}</span>
        {meta && <span className="block truncate text-[12px] text-smoke">{meta}</span>}
      </span>
...
function ProjectRow({ project }: { project: PaletteProjectHit }) {
  return (
    <>
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-[13px] text-snow">{project.name}</span>
        {project.client && <span className="block truncate text-[12px] text-smoke">{project.client}</span>}
      </span>
      {project.archived_at && (
...
function BomRow({ bom }: { bom: PaletteBomHit }) {
  return (
    <>
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-[13px] text-snow">{bom.name}</span>
        {bom.project_name && <span className="block truncate text-[12px] text-smoke">in {bom.project_name}</span>}
      </span>
    </>
  );
}
...
function OrderRow({ order }: { order: PaletteOrderHit }) {
  return (
    <>
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate font-mono text-[13px] text-snow">{order.po_number}</span>
        {order.distributor_name && <span className="block truncate text-[12px] text-smoke">{order.distributor_name}</span>}
      </span>
