...
export function PersonDayModal({
  ...
  return (
    <Drawer open={open} onClose={onClose} width={420} aria-label="Manage day">
      <DrawerHeader>
        <div>
          <div className="text-[17px] text-snow">
      ...
      <DrawerBody className="flex flex-col gap-5">
        {mode === "owner-day" && (
          <Card tone="panel">
            <div className="mb-3 text-[15px] font-medium text-snow">Attendance</div>
        ...
        <div>
          <div className="mb-3 text-[15px] font-medium text-snow">Hours logged</div>
            ...
            <div className="flex flex-col gap-2">
              {existingEntries.map((entry) => (
                <Card key={entry.id} tone="panel" padding="md">
                  {editingId === entry.id ? (
                    <div className="flex flex-col gap-2">
                      <div className="text-[15px] text-snow">{entry.projectName}</div>
                      <Input
                        type="number"
                        step="0.5"
                        min="0"
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="truncate text-[15px] text-snow">{entry.projectName}</div>
                        {entry.note && <div className="truncate text-caption text-smoke">{entry.note}</div>}
                      </div>
                      <div className="flex flex-none items-center gap-2">
                        <span className="font-mono text-[15px] text-silver-mist">{entry.hours}h</span>
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => {
        ...
        <div>
          <div className="mb-3 text-[15px] font-medium text-snow">Add hours</div>
          {projectOptions.length === 0 ? (
