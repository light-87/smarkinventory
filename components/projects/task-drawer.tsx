    ...
    <Drawer open={open} onClose={onClose} aria-label={`Manage task: ${task.title}`}>
      <DrawerHeader>
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate text-[15px] text-snow">{task.title}</div>
        </div>
      ...
      <DrawerBody className="flex flex-col gap-6">
        ...
        {showReminders && projectId && (
          ...
          >
            {openHold && (
              <p className="text-[13px] text-smoke">Awaiting client input since {formatDate(openHold.startedAt)}.</p>
            )}
            <Button
              size="sm"
              variant="outline"
