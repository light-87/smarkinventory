...
export function ProjectHubHeader({ project }: { project: PmProjectView }) {
  return (
    <div className="mb-4">
      <Link href="/projects" className="text-[15px] text-smoke transition-colors hover:text-snow">
        ← All projects
      </Link>
      <div className="mt-2 flex flex-wrap items-center gap-3">
        <h1 className="text-heading-sm font-normal text-snow">{project.name}</h1>
        {project.client && <span className="text-[15px] text-smoke">{project.client}</span>}
