...
export function LeavesThisWeekCard({ leaves, error, nameById }: LeavesThisWeekCardProps) {
  return (
    <Card>
      <div className="mb-4 text-[15px] font-medium text-snow">Leaves this week</div>
      {error || !leaves ? (
        ...
        <ul className="flex flex-col gap-2.5">
          {leaves.map((leave) => (
            <li key={leave.id} className="flex flex-wrap items-center justify-between gap-2 text-[13px]">
