          ...
          {movements.map((m) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className="w-11 flex-none font-mono text-[14px] text-smoke">{m.time}</span>
                {m.pid === "—" ? (
                  <span className="flex-none truncate font-mono text-[15px] text-snow sm:w-28">
                    {m.pid}
                  </span>
                ) : (
                  <Link
                    href={`/part/${m.pid}`}
                    className="flex-none truncate font-mono text-[15px] text-snow transition-colors hover:text-smark-orange sm:w-28"
                  >
                    {m.pid}
                  </Link>
                )}
