    ...
    <>
      <DrawerHeader>
        <div className="min-w-0">
          ...
          {data.part.mpn && <div className="mt-1 truncate font-mono text-[15px] text-silver-mist">{data.part.mpn}</div>}
          {data.part.manufacturer && <div className="mt-0.5 text-[15px] text-smoke">{data.part.manufacturer}</div>}
        </div>
        <div className="flex flex-none items-center gap-3">
          ...
          {variant === "drawer" ? (
            ...
            <button
              ...
              className="cursor-pointer text-[15px] text-smoke hover:text-snow"
            >
              ← Inventory
            </button>
          )}
      ...
      <DrawerBody>
        ...
        {data.part.datasheet_url && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mb-6 inline-block text-[15px] text-smark-orange hover:underline"
          >
            Datasheet ↗
          </a>
        )}
        ...
        {data.timeline.length === 0 ? (
          <p className="py-6 text-center text-[15px] text-smoke">No history yet for this part.</p>
        ) : filteredTimeline.length === 0 ? (
          <p className="py-6 text-center text-[15px] text-smoke">No events match these filters.</p>
        ) : (
