  ...
  return (
    <div className={cn("relative flex-none", className)}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-smark-orange px-1 font-mono text-[12px] font-medium text-white">
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-2 py-1.5">
              <span className="text-[13px] tracking-[0.06em] text-smoke uppercase">Notifications</span>
              {unreadCount > 0 && (
                <button
                  type="button"
                  onClick={() => void markAllRead()}
                  className="cursor-pointer text-[13px] text-smark-orange hover:text-smark-orange-hover"
                >
                  Mark all read
                </button>
              )}
            ...
            {loading && notifications.length === 0 ? (
              <div className="px-2 py-6 text-center text-[15px] text-smoke">Loading…</div>
            ) : notifications.length === 0 ? (
              <div className="px-2 py-6 text-center text-[15px] text-smoke">Nothing yet</div>
            ) : (
              notifications.map((n) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex min-w-0 flex-1 flex-col gap-0.5">
                      <span className="truncate text-[15px] text-snow">{n.title}</span>
                      {n.body && <span className="line-clamp-2 text-[14px] text-smoke">{n.body}</span>}
                      <span className="text-[13px] text-faint">{formatRelativeTime(n.created_at)}</span>
                    </span>
