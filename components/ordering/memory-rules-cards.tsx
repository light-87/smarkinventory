  ...
  return (
    <Card padding="lg">
      <div className="mb-1 flex items-center justify-between gap-3">
        <span className="text-[17px] font-medium text-snow">AI Memory added as context</span>
      ...
      {memory.preview.length === 0 ? (
        <div className="text-[15px] text-smoke">No active rules yet — nothing learned to apply.</div>
      ) : (
        <div className="flex flex-col gap-2">
          {memory.preview.map((rule, i) => (
            <div key={i} className="flex items-baseline gap-2.5">
              ...
              <span className="text-[15px] leading-normal text-silver-mist">{rule.text}</span>
            </div>
          ))}
...
export function StandardRulesCard({ rules }: { rules: StandardRuleRow[] }) {
  return (
    <Card padding="lg" tone="panel">
      <div className="mb-1 flex items-center justify-between gap-3">
        <span className="text-[17px] font-medium text-snow">Standard search rules</span>
      ...
      <div className="flex flex-col">
        ...
        {rules.map((rule) => (
          <div key={`${rule.key}-${rule.rank}`} className="flex items-center gap-3 border-b border-border-hairline py-2.5 last:border-0">
            <span className="flex size-[22px] flex-none items-center justify-center rounded-md bg-ash font-mono text-[13px] text-smoke">
              {rule.rank}
            </span>
            <span className="flex-1 text-[15px] text-silver-mist">{rule.label}</span>
