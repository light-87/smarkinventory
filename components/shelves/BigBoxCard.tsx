  ...
  return (
    ...
    >
      ...
      <div className="border-b border-border-faint bg-surface-hover px-3.5 py-3">
        <div className="truncate font-mono text-[17px] text-snow">{box.code}</div>
        {box.category && (
          <span className="mt-2 inline-block rounded-full border border-charcoal px-2.5 py-0.5 text-[12px] text-silver-mist">
            {box.category}
          </span>
        )}
      </div>
      ...
      <div className="px-2 py-2">
        {box.chips.length === 0 && box.moreCount === 0 ? (
          <div className="px-1.5 py-2 text-[13px] text-faint">Empty box</div>
        ) : (
          <>
            {box.chips.map((chip) => (
              <div key={chip.pid} className="flex items-center gap-1.5 rounded-md px-1.5 py-1">
                ...
                />
                <span className="min-w-0 flex-1 truncate font-mono text-[13px] text-silver-mist">{chip.pid}</span>
                <span
                  className={cn(
            ...
            {box.moreCount > 0 && (
              <div className="px-1.5 pt-1 pb-0.5 text-[13px] text-faint">+ {box.moreCount} more types</div>
            )}
          </>
        )}
      </div>
