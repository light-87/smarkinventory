...
export function AuditFlow({ boxId, boxCode, items, onClose, onFinished }: AuditFlowProps) {
  ...
  return (
    <Drawer open onClose={handlePause} width={440} aria-label={`Audit box ${boxCode}`}>
      <DrawerHeader>
        <div>
          <div className="text-[15px] text-snow">Audit · Box {boxCode}</div>
      ...
      <DrawerBody>
        ...
        {current ? (
          <div className="rounded-2xl border border-charcoal p-5">
            ...
            <div className="mt-1 font-mono text-[13px] text-silver-mist">{current.mpn ?? "—"}</div>
            {current.value && <div className="mt-1 text-[13px] text-smoke">{current.value}</div>}
          ...
          <div className="rounded-2xl border border-dashed border-slate bg-surface-panel py-10 text-center text-[13px] text-smoke">
