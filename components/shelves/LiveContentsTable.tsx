  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-charcoal">
      <div className="flex items-center justify-between border-b border-border-divider px-[18px] py-3.5">
        <span className="text-[15px] font-medium text-snow">Live contents</span>
      ...
      {items.length === 0 ? (
        ...
        items.map((item) => {
          ...
          return (
            ...
            >
              <span className="w-28 flex-none truncate font-mono text-[15px] text-snow">{item.pid}</span>
              <span className="min-w-0 flex-1 truncate font-mono text-caption text-smoke">{item.mpn ?? "—"}</span>
              <span className="w-24 flex-none truncate text-right text-[15px] text-silver-mist">
                {item.value ?? "—"}
              </span>
              <span
                className={cn("w-16 flex-none text-right font-mono text-[15px]", low ? "text-smark-orange" : "text-snow")}
              >
