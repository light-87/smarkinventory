...
export function FilterBar({ filters, options }: FilterBarProps) {
  const selectClass =
    "h-10 w-full rounded-lg border border-charcoal bg-surface-well px-3 text-[15px] text-snow outline-none focus:border-smark-orange sm:w-auto";
  const dateClass =
    "h-10 w-full rounded-lg border border-charcoal bg-surface-well px-3 text-[15px] text-snow outline-none focus:border-smark-orange sm:w-auto";
    ...
    <Card>
      <form method="get" action="/project-dashboard" className="flex flex-col gap-3">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <label className="flex flex-col gap-1 text-[14px] text-smoke">
            From
        ...
        <div className="flex items-center gap-3">
          <button
            type="submit"
            className="flex h-10 items-center justify-center rounded-lg bg-lime px-5 text-[15px] font-medium text-obsidian transition-colors hover:bg-lime-hover"
          >
            Apply filters
          </button>
          <a href="/project-dashboard" className="text-[15px] text-smoke transition-colors hover:text-snow">
            Clear
          </a>
        </div>
      </form>
