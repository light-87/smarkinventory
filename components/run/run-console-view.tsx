    ...
    <Card padding="md" className={lane.rows.some((r) => r.isRecommended) ? "border-smark-orange/50" : undefined}>
      <div className="mb-3 flex items-start justify-between gap-2.5">
        <div className="min-w-0">
          <div className="font-mono text-[15px] text-snow">{lane.ref}</div>
      ...
      {lane.aiSkipReason ? (
        <div className="flex items-center gap-2.5 rounded-lg border border-smark-orange bg-surface-accent-hover px-3.5 py-3">
          ...
          <span className="text-[15px] text-snow">{lane.aiSkipReason}</span>
        </div>
      ) : lane.rows.length > 0 ? (
            ...
            <thead>
              <tr>
                {["Site", "Price", "Stock", "MPN", "Pkg", "Link"].map((h, i) => (
                  <th
                    key={h}
                    className={`px-2 py-1 text-[12px] tracking-[0.04em] text-graphite uppercase ${i === 1 ? "text-right" : i >= 3 ? "text-center" : "text-left"}`}
                  >
                    {h}
                  </th>
                ))}
        ...
        <div className="text-[15px] text-smoke">No listings found across any site in the sequence.</div>
      ) : (
        <div className="text-[15px] text-smoke">Waiting for results…</div>
      )}

      {recommended && !lane.aiSkipReason && (
        <div className="mt-3 border-t border-border-hairline pt-3 text-[15px] leading-[1.5] text-silver-mist">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-[240px] flex-1">
            <div className="mb-3 flex items-center gap-2.5">
              ...
              <span className="text-[17px] text-snow">Master agent</span>
              <Chip tone={tone} size="sm">
                {live.status}
              </Chip>
            </div>
            <div className="font-mono text-[15px] leading-[1.5] text-silver-mist">
