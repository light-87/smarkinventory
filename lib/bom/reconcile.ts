...
import { derivePackageFromFootprint, splitValueVoltage } from "./footprint";
...
/** Per-line reconcile: matches the catalog, then compares `need` to the matched part's stock. */
export function reconcileLine(
  ...
  const { value, voltage } = splitValueVoltage(line.value);
  const hit = matchPart(
    { mpn: line.mpn, lcsc_pn: line.lcsc_pn, value, voltage, package: derivePackageFromFootprint(line.footprint) },
    catalog,
  );
