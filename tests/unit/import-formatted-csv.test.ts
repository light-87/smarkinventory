...
describe("the real client fixture", () => {
  ...
  test("imports 1,745 stock rows from 29 files", () => {
    // The client's v2 drop (2026-08-11), and nothing else. v2 omits every
    // connector file: Krunal Sir is reworking those for "proper
    // differentiation" and asked explicitly that no v1 data survive the
    // changeover ("Do not have any old data"), so they are absent rather than
    // carried over. Fresh connector files arrive as their own drop.
    expect(result.fileSummary).toHaveLength(29);
    expect(result.parts).toHaveLength(1745);
  });
  ...
  test("every row gets a category, and they are all mapped values", () => {
    ...
    // 29 files collapse to 26 categories: ic_smd+ic_th → IC, misc_switch +
    // misc_rocker_switch → Switch, misc_lcd+misc_oled → Display.
    expect(new Set(result.parts.map((p) => p.category)).size).toBe(26);
  });
  ...
  test("1,622 rows carry a usable positive quantity", () => {
    // Plain numbers plus the 9 text quantities, each of which still yields a
    // leading count ("16 strip" → 16). The remaining 123 are a genuine 0 or
    // blank, and blank means unknown rather than zero.
    expect(result.parts.filter((p) => (p.qty ?? 0) > 0)).toHaveLength(1622);
  });

  test("repeated MPNs are reported for a human call, not merged away", () => {
    // 32 collisions: the same part number typed onto two genuinely different
    // rows. Exactly the case that makes MPN the wrong dedupe key here.
    const duplicates = collectDuplicateMpns(result.parts);
    expect(duplicates).toHaveLength(32);
    // "LED GREEN" is the archetype: three rows in misc_led.csv holding 100,
    // 830 and 85 pieces. They are separate entries the client keeps apart, so
    // merging on the part number would both lose that distinction and silently
    // restate the quantity on hand.
    const ledGreen = duplicates.find((d) => d.mpn === "LED GREEN");
    expect(ledGreen?.occurrences).toHaveLength(3);
  });

  test("the provenance key is unique across all 1,745 rows", () => {
    ...
    //   sheet#row  — 29 collisions (side-by-side tables share row numbers)
    //   file|row   — none in this drop, but connectors.csv merged two sheets
    //                in v1 and the reworked connector files may do so again
    ...
    expect(new Set(keys).size).toBe(1745);
  });
  ...
  test("the two simpler keys really do collide — this is why the full key exists", () => {
    ...
    expect(new Set(sheetRow).size).toBe(1745 - 29);

    const fileRow = result.parts.map((p) => `${p.source_file}|${p.source_row}`);
    expect(new Set(fileRow).size).toBe(1745);
  });
